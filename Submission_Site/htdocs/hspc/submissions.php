...
<html>
	...
	<body>
		<div id="header">
			...
			<div class="navbutton">
				<a href="submissions.php">DHS HSPC</a>
			</div>
			<div class="navbutton">
				<a href="/hspc/api/index.html">Java API</a>
			</div>
		</div>
		<div id="logout">
		...
		<div id="content" style="display: inline-block; text-align: center;">
			...
			<div id="small">
				...
				<?php
					if (isset($_GET["code"])) {
						...
						if ($code == 0) {
							...
							// The team already completed this problem
							echo "<h5 class='text'>Problem already completed correctly</h5><hr>";
						}
					}
				?>
				</div>
				...
				<script>
					...
					function doSubmit() {
						if (checkSelect() == true) {
							if (document.getElementById("submission").files.length > 0) {
				...
				<form action="submit.php" method="post" enctype="multipart/form-data" id="submitForm">
					<input name="submission[]" type="file" multiple="multiple" id="submission" class="noborder text" requried/>
					<br>
					<select name="problemNumber" style="width:100%; margin-top:8px; margin-bottom:8px; padding:8px;" id="problemNumberSelect" onchange="checkSelect()">
						<!--<option value="99" selected>Problem 0 - Practice</option>-->
